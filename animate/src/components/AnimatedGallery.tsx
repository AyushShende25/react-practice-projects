  ...
  return (
    <div className="w-[80%] flex">
      {images.map((image, index) => (
        <motion.div
          key={index}
          className="relative overflow-hidden shadow-lg rounded-lg m-[1rem]"
          whileHover={{ scale: 1.05 }}
        >
          <img src={image.src} alt={image.caption} className="w-full h-auto" />
          <motion.div
            whileHover={{ opacity: 1 }}
            className="absolute inset-0 bg-black text-white flex items-center justify-center bg-opacity-50 opacity-0 hover:opacity-100 transition-opacity"
          >
